...
public class XxlJobServiceImpl implements XxlJobService {
        ...
        if (jobInfo.getChildJobId() != null && jobInfo.getChildJobId().trim().length() > 0) {
            ...
            for (String childJobIdItem : childJobIds) {
                if (childJobIdItem != null && childJobIdItem.trim().length() > 0 && isNumeric(childJobIdItem)) {
                    XxlJobInfo childJobInfo = xxlJobInfoMapper.selectById(Long.valueOf(childJobIdItem));
    ...
    public ReturnT<String> update(XxlJobInfo jobInfo) {
        ...
        // stage job info
        XxlJobInfo exists_jobInfo = xxlJobInfoMapper.selectById(jobInfo.getId());
    ...
    public ReturnT<String> remove(Long id) {
        XxlJobInfo xxlJobInfo = xxlJobInfoMapper.selectById(id);
        if (xxlJobInfo == null) {
            return ReturnT.SUCCESS;
        }
        xxlJobInfoMapper.delete(Wrappers.<XxlJobInfo>lambdaQuery().eq(XxlJobInfo::getId, id));
        xxlJobLogMapper.delete(Wrappers.<XxlJobLog>lambdaQuery().eq(XxlJobLog::getJobId, id));
